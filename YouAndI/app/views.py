...
def test_load(request, test_pk, guest_pk):
    ...
    return render(request, 'user2_test.html', {'test': test, 'guest':guestObj, 'testJson':testJson, 'truetag':truetag,})


def test_result(request, test_pk, gtest_pk):
    test = Test.objects.get(pk=test_pk)
    ...
    return render(request, 'result.html', {'test':test, 'gtest':gtest,'result':result})
